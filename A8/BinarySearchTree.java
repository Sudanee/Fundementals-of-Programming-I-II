...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	public TreeNode<K,V> getSibling(K key){
		TreeNode<K,V> n = root;
		return getSiblingHelper(n, key);
	}

	private TreeNode<K,V> getSiblingHelper(TreeNode<K,V> n, K key){
		...
		if (n == null){
		...
		else if (left.key.compareTo(key) == 0){
			return right;
		}
			...
			return getSiblingHelper(left, key);
		}
		else if (right.key.compareTo(key) > 0){
			return getSiblingHelper(right, key);
		}
	}
}